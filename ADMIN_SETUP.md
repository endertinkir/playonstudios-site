...
- `/functions/index.js`
  - rolls cumulative `users/{uid}` ad counters into daily `userDailyAdMetrics` deltas
...
7. Deploy or paste the Firestore rules into Firebase. If you want daily ad deltas, deploy the Cloud Function too.
...
### `userDailyAdMetrics/{YYYY-MM-DD__platform__country__build__segment__uid}`

Generated by `rollupUserDailyAdMetrics` from cumulative `users/{uid}` ad fields. The user document stays lifetime/cumulative; this collection stores positive per-update deltas for daily dashboard reporting.
...
```json
{
  ...
  "country": "US",
  "buildNumber": "18",
  "installDateKey": "2026-06-04",
  "segment": "casual",
  "interstitialAdWatchCount": 4,
  "rewardedAdWatchCount": 2,
  "interstitialPaidImpressionCount": 5,
  "rewardedPaidImpressionCount": 1,
  "interstitialAdRevenueMicros": 12000,
  "rewardedAdRevenueMicros": 8000,
  "interstitialAdTotalVisibleMs": 18000,
  "rewardedAdTotalVisibleMs": 32000,
  "interstitialAdShortCloseCount": 0,
  "rewardedAdNoRewardCloseCount": 1,
  "interstitialAdInterruptedCount": 0,
  "rewardedAdInterruptedCount": 0,
  "excludedFromDashboard": false,
  "updatedAt": "serverTimestamp"
}
```

Revenue is always displayed as `RevenueMicros / 1,000,000`. This rollup is forward-looking: it starts when the function is deployed and does not reconstruct historical daily values from existing cumulative user totals.

For Unity migration safety, `rollupUserDailyAdMetrics` skips the first positive ad-counter write on users that have `legacyUid`. That write is treated as imported lifetime baseline, not as same-day ad revenue. If a legacy baseline was already written before this guard existed, mark the affected `userDailyAdMetrics` rows with `excludedFromDashboard: true`; the dashboard will ignore those rows without deleting audit history. Use the cleanup helper in dry-run mode first:

```bash
node scripts/exclude-legacy-ad-baselines.js --date=2026-07-22
node scripts/exclude-legacy-ad-baselines.js --date=2026-07-22 --apply
```

The admin dashboard separates date semantics:

- **Event window** filters when activity or ad deltas happened.
- **Install cohort** filters which install-date users are included in those event-window metrics.

User lookup reads from the already-loaded `users` snapshot first. If an exact user ID is not loaded, the panel fetches only that one `users/{uid}` document.

Deploy the function and rules together:
